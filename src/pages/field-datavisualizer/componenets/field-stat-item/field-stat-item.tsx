...
function getBody(type: typeof FIELD_TYPES[keyof typeof FIELD_TYPES], totalCount: number, data?: FieldStatData, fieldName?: string) {
    if (type === 'date')
        return (
            <>
                {data?.earliest && <div>Самая раняя дата: {moment.unix(data.earliest / 1000).format("DD-MM-YYYY HH:mm:ss")}</div>}
                {data?.latest && <div>Самая поздняя дата: {moment.unix(data.latest / 1000).format("DD-MM-YYYY HH:mm:ss")}</div>}
            </>
        )
    if (type === 'text')
        return (
...
function getColor(type: string): 'primary' | 'secondary' | 'danger' | 'subdued' | 'accent' {
